...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    for (const file of files) {
      ...
      blogs.push({ id: file.replace(/\.json$/, ''), title: blog.title, content: blog.content });
    }
    blogs.sort((a, b) => b.id.localeCompare(a.id)); // 新的在前
    res.json({ blogs });
    return;
  }
  ...
  if (req.method === 'POST') {
    ...
    await fs.writeFile(path.join(BLOG_DIR, id + '.json'), JSON.stringify({ title, content }, null, 2));
    res.json({ ok: true, id });
    return;
  }
